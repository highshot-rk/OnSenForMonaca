<template>
  <v-ons-page>
    <v-ons-toolbar class="header">
        ...
        <div class="right" style="width: 70%;">
          <v-ons-button
            ...
            class=" h_right"
            @click="clickDelete()"
          ><span class="material-icons">content_copy</span></v-ons-button>
    ...
    <template v-for="(number, index) in menus">
      ...
      :done="index%2 == 1 ? true : false"
      @start-practice="clickStart()"
      @skip-practice="clickSkip()"
      @redo-practice="clickRedo()"
      ></menu-card>
    </template>
    ...
    <v-ons-alert-dialog modifier="rowfooter"
      :visible.sync="confirm_start_practice"
    >
      <p slot="title" class="title" style="font-weight: bold;">練習を開始しますか？</p>
      <span class="content">選択したメニューから練習を開始します。よろしいですか？</span><br/>
      <span>よろしいですか？。</span>
      <template slot="footer">
        <v-ons-alert-dialog-button @click="confirm_start_practice = false"><span class="cancel">キャンセル</span></v-ons-alert-dialog-button>
        <v-ons-alert-dialog-button @click="confirm_start_practice = false"><span class="ok">はい</span></v-ons-alert-dialog-button>
      </template>
    </v-ons-alert-dialog>

    <v-ons-alert-dialog modifier="rowfooter"
      :visible.sync="confirm_skip"
    >
      <p slot="title" class="title" style="font-weight: bold;">メニュー#2を省略</p>
      <span class="content">メニュー#2の再生をスキップします。このメニューは消去されず、実施済として処理されます。</span><br/>
      <span>よろしいですか？。</span>
      <template slot="footer">
        <v-ons-alert-dialog-button @click="confirm_skip = false"><span class="cancel">キャンセル</span></v-ons-alert-dialog-button>
        <v-ons-alert-dialog-button @click="confirm_skip = false"><span class="ok">省略する</span></v-ons-alert-dialog-button>
      </template>
    </v-ons-alert-dialog>

    <v-ons-alert-dialog modifier="rowfooter"
      :visible.sync="confirm_delete"
    >
      <p slot="title" class="title" style="font-weight: bold;">メニューの削除</p>
      <span class="content">メニュー#本当に「2021.03.17月の練習」</span><br/>
      <span>を削除しますか？</span>
      <template slot="footer">
        <v-ons-alert-dialog-button @click="confirm_delete = false"><span class="cancel">キャンセル</span></v-ons-alert-dialog-button>
        <v-ons-alert-dialog-button @click="confirm_delete = false"><span class="ok" style="color: #EA3223;">削除</span></v-ons-alert-dialog-button>
      </template>
    </v-ons-alert-dialog>

    <v-ons-alert-dialog modifier="rowfooter"
      :visible.sync="confirm_redo"
    >
      <p slot="title" class="title" style="font-weight: bold;">メニュー#2をやり直す</p>
      <span class="content">メニュー#2をやり直しますか？すでに保存されているタイムがある場合、そのタイムは全て消去されます。</span><br/>
      <template slot="footer">
        <v-ons-alert-dialog-button @click="confirm_redo = false"><span class="cancel">キャンセル</span></v-ons-alert-dialog-button>
        <v-ons-alert-dialog-button @click="confirm_redo = false"><span class="ok" style="font-weight: bold;">やり直す</span></v-ons-alert-dialog-button>
      </template>
    </v-ons-alert-dialog>
  </v-ons-page>
</template>
...
export default {
  ...
  methods: {
    ...
    clickStart() {
      this.confirm_start_practice = !this.confirming_practice;
    },
    clickSkip() {
      this.confirm_skip = !this.confirm_skip;
    },
    clickDelete() {
      this.confirm_delete = !this.confirm_delete;
    },
    clickRedo() {
      this.confirm_redo = !this.confirm_redo;
    }
  }
}
</script>
