    ...
    <v-ons-page style="height: 100vh">
        <v-ons-toolbar class="practice-toolbar">
            ...
            <div class="center" style="width:30%">
                <p class="practice-toolbar-title text-font-18">メニュー内容リスト</p>
            </div>
        ...
        <v-ons-card class="card-item" style="margin-top: 2.9rem; margin-bottom: 0;">
            <v-ons-list>
                <v-ons-list-item v-for="menu_content, index in this.menu_content_list" :key="index">
                    <div class="left"><span class="left-list text-font-16">{{menu_content.name}}</span></div>
                    <div class="center center-list text-font-12">
                        <div style="float:right; width:fit-content; margin-left:auto;margin-right:0;">
                            <span>目標タイム</span><br>
                            <span style="text-align:left;">+{{menu_content.target_time}}秒</span>
                        </div>
                    </div>
                    <div class="right">
            ...
            <v-ons-button modifier="quiet" style="float:left; margin-top:1rem;"
                        @click="addcontentDialogVisible=true"
            >
            ...
            >
                <span slot="title" class="dialog-title text-font-17">{{this.menu_content_list[current_selected_content].name}}の削除</span>
                <p class="dialog-text text-font-13">本当に「{{this.menu_content_list[current_selected_content].name}}」を削除しますか？</p> 
                ...
                <template slot="footer">
                    ...
                    <v-ons-alert-dialog-button @click="deletecontent" class="dialog-button text-font-16 dialog-delete-button-color">削除</v-ons-alert-dialog-button>
                </template>
            </v-ons-alert-dialog>
            ...
            >
                <span slot="title" class="dialog-title">新規内容の追加</span>
                ...
                <p class="dialog-sub-title text-font-12" style="margin-top:1rem">目標タイム</p>
                
                <v-ons-row class="left-list" style="color:black;">
                    <v-ons-col width="40%" style="display: table;margin:auto; text-align: right;">
                        <span style="display: table-cell;vertical-align:middle">ベストタイム</span>
                    </v-ons-col>
                    <v-ons-col width="5%" style="display: table;margin:auto;">
                        <span style="margin:auto;">+</span>
                    </v-ons-col>
                    <v-ons-col width="40%" style="display: table;margin:auto;">
                        <v-ons-select class="dialog-text grey-btn" style=" margin:0; width:100%"></v-ons-select>
                    </v-ons-col>
                </v-ons-row>
                <template slot="footer">
...
export default {
    ...
    data: function() {
        return {
            menu_content_list:[
                {"name":"ヘッドアップ", "target_time":3},
                {"name":"ビルドアップ", "target_time":4},
                {"name":"デセンディング", "target_time":5},
            ],
            current_selected_content: 0,
    ...
    methods: {
        confirmDelete: function (index) {
            ...
            this.current_selected_content=index;
        },
